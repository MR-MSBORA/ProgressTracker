...
const SkillCard = ({ skill }) => {
  ...
  const progressPercentage = skill.targetHours > 0 
    ? Math.min(Math.round((skill.totalHours / skill.targetHours) * 100), 100)
    : 0;
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`px-3 py-1 text-xs rounded-full ${levelColors[skill.currentLevel] || levelColors.beginner}`}>
                {skill.currentLevel}
              </span>
            </div>
          </div>
            ...
            {showMenu && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-10">
                <button
                  ...
                  className="w-full flex items-center gap-2 px-4 py-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"
                >
                  <FiTrash2 className="w-4 h-4" />
                  Delete
                </button>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-600 dark:text-gray-400">
              {skill.totalHours.toFixed(1)}h / {skill.targetHours}h
            </span>
          </div>
          ...
          <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
            {skill.targetHours - skill.totalHours > 0 ? (
              <span>{(skill.targetHours - skill.totalHours).toFixed(1)}h remaining to reach target</span>
            ) : (
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            <FiClock className="w-4 h-4" />
            <span>{skill.practiceLogs?.length || 0} sessions</span>
          </div>
