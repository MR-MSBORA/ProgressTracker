...
//   const progress = dailyScore.maxScore > 0 
...
//           </div>
//           <p className="text-xl font-bold text-gray-900 dark:text-white">{dailyScore.pendingTasks}</p>
//         </div>
...
const DailyScore = () => {
  ...
  const maxScore = dailyScore?.maxScore || 0;
  const percentage = maxScore > 0 ? Math.round((score / maxScore) * 100) : 0;
  const completed = dailyScore?.completedTasks || 0;
  const pending = dailyScore?.pendingTasks || 0;
  const total = dailyScore?.totalTasks || 0;
